...
const TermsModal = ({ isOpen, onClose }) => {
  ...
  return (
    // Fixed overlay/backdrop, allowing close on backdrop click
    <div
    ...
    >
      ...
      <div
        className="bg-white max-w-4xl w-11/12 max-h-[90vh] rounded-xl shadow-2xl overflow-y-auto scrollbar-hide relative"
        onClick={handleContentClick}
      >
        {/*
          *** X ICON / CLOSE BUTTON IS HERE ***
          It is positioned absolutely in the top right corner (top-4 right-4)
          and has a high z-index (z-10) to stay above the scrollable content.
        */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-gray-500 hover:text-gray-900 z-10 p-2 transition-colors bg-white rounded-full shadow-md"
          aria-label="Close modal"
        >
          <svg
            className="w-6 h-6 cp"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
        ...
        <div className="overflow-y-auto p-8 sm:p-12 lg:p-16 h-full custom-scrollbar">
          <div
      ...
      {/* Custom styles for consistent look and scroll handling */}
      <style jsx="true">{`
        /* Crucial: Disable smooth scrolling on the main HTML body when using a modal,
           and enable scrolling *only* on the modal content area for jump links to work correctly within the modal */
        html {
        ...
        .terms-content-styles h2 {
          ...
          margin-top: 3rem;
          margin-bottom: 1rem;
          /* Padding for anchor link target offset below the sticky close button */
          padding-top: 1.5rem; 
        }
